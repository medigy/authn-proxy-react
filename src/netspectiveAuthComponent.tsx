...
export const netspectiveAuthentication = (parms: IParms): Promise<IReturns> => {
  ...
  keycloakUrlEncoded.append(
    "client_id",
    String(process.env.REACT_APP_KEYCLOAK_CLIENTID)
  );
  keycloakUrlEncoded.append(
    "client_secret",
    String(process.env.REACT_APP_KEYCLOAK_CLIENTSECRET)
  );
    ...
    if (authProvider === "gitlab") {
      if (type === "login") {
        if (username && password) {
          ...
          return fetchAction(
            String(process.env.REACT_APP_GITLAB_AUTHURL),
            urlencoded
          ).then((responseVal) => {
      ...
      if (type === "login") {
        if (username && password) {
          ...
          return fetchAction(authUrl, keycloakUrlEncoded).then(
            (responseVal) => {
              if (responseVal.statusText) {
                return Promise.resolve({
                  status: Number(responseVal.status),
                ...
                localStorage.setItem(
                  "refresh_token",
                  responseVal.refresh_token
                );
                return fetchActionKeyCloakUserInfo(
                  userInfoUrl,
      ...
      if (username && password) {
        ...
        return fetchAction(
          ...
          if (responseValGit.statusText) {
            ...
            localStorage.setItem(
              "gitlab_access_token",
              responseValGit.access_token
            );
            localStorage.setItem("gitlab_id_token", responseValGit.id_token);
            localStorage.setItem(
              "gitlab_refresh_token",
              responseValGit.refresh_token
            );
            return fetchAction(authUrl, keycloakUrlEncoded).then(
              (responseValKey) => {
                localStorage.setItem(
                  "access_token",
                  responseValKey.access_token
                );
                localStorage.setItem(
                  "refresh_token",
                  responseValKey.refresh_token
                );
                return Promise.resolve({
                  status: 200,
                  message: "login_successful",
